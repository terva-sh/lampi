...
updated_at: 2026-09-24T07:24:29Z
created_by:
...
- [x] A stored cursor_cli_store_json export projects to schema_version 1 events. session_id is cursor-cli: plus the native session id. A cursor_state_json or transcript_jsonl blob on a cursor-cli session is a failure. The CAS object is not opened for write
...
- [x] go test ./... is green, including a unit or accept-style cursor_cli_store_json fixture. TestUnimplementedHarnessRecordsNormalizeError no longer expects a valid Cursor CLI export to fail
...
normalize.CursorCLI projects a stored Cursor CLI export onto schema_version 1 events. The blob is artifact kind cursor_cli_store_json. session_id is cursor-cli: plus the native session id, the chat directory chats/<workspace>/<session>. api.Server.Project calls it for harness cursor-cli and reads that kind only. It does not call the IDE projector and does not open store.db.

Unknown keys stay on the event. encrypted, cipher, and sealed fields, including encrypted_content, are copied onto extra, left out of content_text, and not decrypted. Credential keys stay absent. Meta key "0" is a session meta row. Other meta objects are meta only when every field is a name, title, or timestamp. A blob is a message only when it has cleartext and a user or assistant role, or numeric type 1 or 2. Tool calls, tool results, usage, and compaction stay unknown. harness_version stays the adapter's pinned reader version. schema_version stays 1.

A cursor_state_json or transcript_jsonl artifact on a cursor-cli session sets normalize_error, and so does a body that is not a meta/blobs document. The error text does not include the body. The derived JSONL and parquet are removed, and the CAS object stays in place. An empty conversation still succeeds.

A successful worker clears normalize_error and writes normalized/<session_uid>.jsonl plus parquet/date=YYYY-MM-DD/harness=cursor-cli/<session_uid>.parquet. The date is the UTC day of recorded_at, or ingested_at when the record has no time. terva-lampi export --format events writes the projected events. Allowlisted sharegpt and trajectory exports follow the existing training rules. The Cursor CLI adapter is unchanged. go test ./... passed.

docs/architecture.md now names terva, Claude Code, Codex CLI, OpenCode, Cursor IDE, and Cursor CLI as schema_version 1 projectors. That edit is the epic's documentation criterion and landed with this last child. The epic ticket stays ready because TKT-01M38RJTB89GY6GWC3XZZFYYY9 (Cursor IDE normalize projector) is still ready in the store.
