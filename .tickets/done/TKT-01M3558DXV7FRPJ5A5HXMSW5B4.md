...
POST /v1/manifests stores the catalog row, enqueues a normalize job, and returns the ACK without projecting. Two in-process workers claim jobs, read the catalog head, and call the existing projector. When this ticket landed, only terva was implemented, and Claude and Codex recorded normalize_error once a worker ran. Workers now project terva, claude, codex, opencode, and cursor. A stored Cursor CLI manifest still records normalize_error. A generation on the session drops a stale publish when a newer ingest is queued. The job row stays until that publish, so a restart finishes it. Shutdown drains the queue.
...
When this ticket landed, terva projected and Claude and Codex had no projector; the worker recorded that, not the ACK handler. Workers now project terva, claude, codex, opencode, and cursor. A stored Cursor CLI manifest still records normalize_error. internal/accept TestMVPAcceptance passes.
