...
- Normalize worker. A transient catalog error drops the job from memory until the next restart (`runNormalize` returns early without a requeue at `internal/api/worker.go:151-157` and `:166-172`). A transient CAS read error becomes a permanent `normalize_error`. `s.pubs` grows by one mutex per session.
