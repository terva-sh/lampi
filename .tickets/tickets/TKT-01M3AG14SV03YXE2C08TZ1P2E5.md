...
references: []
claim:
  actor: agent:cursor/0888
  branch: cursor/cursor-1b-workspace-export-0888
  worktree: /workspace
  commit: 7ac8f6bc5a39dba8c99be1da28e0ac0e092b97c3
  session: null
  claimed_at: 2026-09-24T19:59:49Z
  expires_at: null
archive: null
created_at: 2026-09-24T19:59:39Z
updated_at: 2026-09-24T19:59:49Z
created_by:
...
- [ ] A workspace cursor_state_json snapshots global state.vscdb read-only and merges cursorDiskKV rows for composers named by composer.composerHeaders
- [ ] Session id stays workspace/<id>, and a global database alone is still not uploaded
- [ ] Composers from other workspaces, and composers listed only on composer.composerData, are absent from the allowlisted export
- [ ] Adapter Version is bumped, confidence stays low, and cursorAuth/* stays stripped
- [ ] export --format events on an allowlisted workspace whose bubbles live only on the global database yields type 1 and type 2 content_text, and session_id is cursor:workspace/<id>
- [ ] A wrong artifact kind records normalize_error, and encrypted, cipher, and sealed values stay out of content_text
- [ ] go test ./... is green with a dual-DB fixture and does not open a live Cursor install

## Definition of done

- [ ] Enrichment lives in the existing Cursor harness adapter
- [ ] The dual-DB fixture locks composer.composerHeaders and does not treat composer.composerData as the registry
- [ ] Soft-link, deploy, AgentsView, toolFormerData promotion, cursor-cli store.db, live DB opens, and PlantCursor are unchanged
