...
| Normalize | `internal/normalize` | Workers project terva, claude, codex, and opencode onto schema_version 1 events. Unknown fields kept. `encrypted_content` stays opaque. Parquet is partitioned by UTC date and harness. Cursor IDE (`cursor`) and Cursor CLI (`cursor-cli`) still record `normalize_error` |
...
is the event harness (`terva`, `claude`, `codex`, or `opencode`). The
file name is the session uid, so a re-projection replaces that session
and leaves the rest of the day in place. DuckDB reads the tree with
...
| Package | Later work |
|---------|------------|
| `internal/normalize` | Cursor IDE (`cursor` / `cursor_state_json`) and Cursor CLI (`cursor-cli`) still record `normalize_error` until those child tickets land. terva, claude, codex, and opencode already project onto schema_version 1 |
...
interpret. Normalize workers project terva, claude, codex, and opencode
onto schema_version 1. A Cursor IDE manifest (`cursor` /
`cursor_state_json`) or a Cursor CLI manifest (`cursor-cli`) is stored,
and a worker sets `normalize_error` until that child ticket lands.
Retiring the remaining terva-only acceptance criterion waits on the last
of those children, Cursor CLI. Path-based
